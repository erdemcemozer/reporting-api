package com.example.reportingapi.constants;

/**
 * @Author Erdem Ozer
 * Date: 04/11/2023
 */
public class ReportingConstants {

    public static final String TEST_URL_STRING = "https://sandbox-reporting.rpdpymnt.com/api/v3";
    public static final String CONTENT_TYPE_HEADER_NAME = "Content-Type";
    public static final String CONTENT_TYPE_HEADER_VALUE = "application/json";
    public static final String JSON_FIELD_TOKEN = "token";

    /** ENDPOINTS */
    public static final String USER_LOGIN_ENDPOINT = "merchant/user/login";
    public static final String TRANSACTION_REPORT_ENDPOINT = "transactions/report";
    public static final String TRANSACTION_LIST_ENDPOINT = "transaction/list";
    public static final String TRANSACTION_ENDPOINT = "transaction";
    public static final String CLIENT_ENDPOINT = "client";
    /** ENDPOINTS */

}
